...
export const reducer = (state: T.State = initialState, action: T.Actions) => {
  switch (action.type) {
    case '@listidCardids/set': // 목록uuid의 속성에 카드uuid의 배열을 추가, 계산된 속성명
      return {...state, [action.payload.listed]: action.payload.cardids}
    ...
    case '@listidCardids/prependCardid': {
      const cardids = state[action.payload.listed]
      return {...state, [action.payload.listed]: [action.payload.cardids[0], ...cardids]}
    }
    case '@listidCardids/appendCardid': {
      // 계산된 속성명은 추가 뿐만 아니라 update도 가능하다.
      const cardids = state[action.payload.listed]
      return {...state, [action.payload.listed]: [...cardids, action.payload.cardids[0]]}
    }
    case '@listidCardids/removeCardid': {
      const cardids = state[action.payload.listed]
      return {
        ...state,
        [action.payload.listed]: cardids.filter(
          id => !action.payload.cardids.includes(id)
        )
      }
    }
  }
